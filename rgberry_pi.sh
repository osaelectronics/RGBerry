...
productname="RGBerry Series Board" # the name of the product to install
scriptname="rgberry_pi" # the name of this script
...
topdir="OSAElectronics" # the name of the top level directory
localdir="na" # the name of the dir for copy of resources
gitreponame="RGBerry" # the name of the git project repo
gitusername="osaelectronics" # the name of the git user to fetch repo from
...
GETPOOL="http://get.osaelectronics.com"
...
get_install() {
    ...
    curl -sS http://get.osaelectronics.com/$1 | bash -s - "-y" $2
...
apt_deb_install() {
    ...
    if [[ "$1" != *".deb"* ]]; then
        ...
        DEBDIR=`mktemp -d /tmp/osaelectronics.XXXXXX`
        cd $DEBDIR
...
disable_pi_audio() {
    if grep -q "^dtparam=audio=on" $CONFIG; then
        sudo sed -i "/^dtparam=audio=off$/ s|^|#|" $CONFIG
...
set_rgberry_video() {
	if grep -q "overscan_scale=1" $CONFIG; then
        echo -e "Already active"
    else
        echo "overscan_scale=1" | sudo tee -a $CONFIG &> /dev/null
    fi	

    if grep -q "disable_overscan=1" $CONFIG; then
        echo -e "Already active"
    else
        echo "disable_overscan=1" | sudo tee -a $CONFIG &> /dev/null
    fi
	...
	if grep -q "config_hdmi_boost=0" $CONFIG; then
        echo -e "Already active"
    else
        echo "config_hdmi_boost=0" | sudo tee -a $CONFIG &> /dev/null
    fi
	...
	if grep -q "hdmi_drive=2" $CONFIG; then
        echo -e "Already active"
    else
        echo "hdmi_drive=2" | sudo tee -a $CONFIG &> /dev/null
    fi	
	
	if grep -q "boot_delay=0" $CONFIG; then
        echo -e "Already active"
    else
        echo "boot_delay=0" | sudo tee -a $CONFIG &> /dev/null
    fi		
	...
	if grep -q "avoid_safe_mode=1" $CONFIG; then
        echo -e "Already active"
    else
        echo "avoid_safe_mode=1" | sudo tee -a $CONFIG &> /dev/null
    fi	

	if grep -q "framebuffer_depth=32" $CONFIG; then
        echo -e "Already active"
    else
        echo "framebuffer_depth=32" | sudo tee -a $CONFIG &> /dev/null
    fi	

	if grep -q "framebuffer_ignore_alpha=1" $CONFIG; then
        echo -e "Already active"
    else
        echo "framebuffer_ignore_alpha=1" | sudo tee -a $CONFIG &> /dev/null
    fi	

	if grep -q "enable_dpi_lcd=1" $CONFIG; then
        echo -e "Already active"
    else
        echo "enable_dpi_lcd=1" | sudo tee -a $CONFIG &> /dev/null
    fi	

	if grep -q "display_default_lcd=1" $CONFIG; then
        echo -e "Already active"
    else
        echo "display_default_lcd=1" | sudo tee -a $CONFIG &> /dev/null
    fi	

	if grep -q "dpi_output_format=6" $CONFIG; then
        echo -e "Already active"
    else
        echo "dpi_output_format=6" | sudo tee -a $CONFIG &> /dev/null
    fi		
	
	if grep -q "dpi_group=2" $CONFIG; then
        echo -e "Already active"
    else
        echo "dpi_group=2" | sudo tee -a $CONFIG &> /dev/null
    fi	
	
	if grep -q "dpi_mode=87" $CONFIG; then
        echo -e "Already active"
    else
        echo "dpi_mode=87" | sudo tee -a $CONFIG &> /dev/null
    fi	
	
	if grep -q "hdmi_timings=384 1 0 11 28 224 1 30 0 30 0 0 0 60 0 6400000 1" $CONFIG; then
        echo -e "Already active"
    else
        echo "hdmi_timings=384 1 0 11 28 224 1 30 0 30 0 0 0 60 0 6400000 1" | sudo tee -a $CONFIG &> /dev/null
    fi	
}

set_rgberry_audio() {
	if grep -q "audio_pwm_mode=2" $CONFIG; then
        echo -e "Already active"
    else
        echo "audio_pwm_mode=2" | sudo tee -a $CONFIG &> /dev/null
    fi
	
	if grep -q "disable_audio_dither=1" $CONFIG; then
        echo -e "Already active"
    else
        echo "disable_audio_dither=1" | sudo tee -a $CONFIG &> /dev/null
    fi
	
	if grep -q "dtoverlay=audremap,pins_18_19" $CONFIG; then
        echo -e "Already active"
    else
        echo "dtoverlay=audremap,pins_18_19" | sudo tee -a $CONFIG &> /dev/null
    fi	
}
...
set_rgberry_i2c_alt() {
	...
	if grep -q "dtoverlay=i2c-gpio,i2c_gpio_sda=10,i2c_gpio_scl=11" $CONFIG; then
        echo -e "Already active"
    else
        echo "dtoverlay=i2c-gpio,i2c_gpio_sda=10,i2c_gpio_scl=11" | sudo tee -a $CONFIG &> /dev/null
    fi
}
...
if $IS_EXPERIMENTAL; then
    ...
    warning "https://www.osaelectronics.com/forums/ if you experience issues with this product.\n"
fi
    ...
    if [ $gpioreq == "yes" ]; then
        ...
        if apt_pkg_req "raspi-gpio"; then
            if ! apt_pkg_install "raspi-gpio" &> /dev/null; then
                ...
                DEBDIR=`mktemp -d /tmp/osaelectronics.XXXXXX` && cd $DEBDIR
    ...
    if [ -f "$(which python2)" ] && [ $pip2support == "yes" ] && apt_pkg_req "python-$debpackage" &> /dev/null; then
        if [ $piplibname != "na" ] && [ $debpackage == "na" ]; then
            ...
            if ! sudo -H $PIP2_BIN install "$piplibname"; then
                ...
                echo "If problems persist, visit https://www.osaelectronics.com/forums/ for support"
                exit 1
            fi
        fi
    fi
    ...
    if [ -f "$(which python3)" ] && [ $pip3support == "yes" ] && apt_pkg_req "python3-$debpackage" &> /dev/null; then
        if [ $piplibname != "na" ] && [ $debpackage == "na" ]; then
            ...
            if ! sudo -H $PIP3_BIN install "$piplibname"; then
                ...
                echo "If problems persist, https://www.osaelectronics.com/forums/ for support"
                exit 1
            fi
        fi
    fi
...
# resources install
    ...
    if ! $MIN_INSTALL && [ -n "$copydir" ]; then
        ...
        TMPDIR=`mktemp -d /tmp/osaelectronics.XXXXXX`
        cd $TMPDIR
...
# script custom routines
    ...
    if [ $customcmd == "no" ]; then
        ...
        echo -e "\nFinalising Install...\n"
		add_dtoverlay dtoverlay=rgb-pi
        if [ -e $CONFIG ] && grep -q -E "^dtparam=audio=on$" $CONFIG; then
            ...
            bcm2835off="no"
            echo -e "Disabling default sound module and using onboard one"
            sudo sed -i "/^snd-bcm2835$/ s|^|#|" $LOADMOD
            ASK_TO_REBOOT=true
        else
